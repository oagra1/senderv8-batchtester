function getCountryByPhoneNum(participantsInfoList) {
  for (let i = 0; i < participantsInfoList.length; i++) {
    let phoneNum = participantsInfoList[i]['phoneNum']
    let PhoneNumber = require('awesome-phonenumber')
    // 获取手机号的方法必须要用带+号的手机号
    let c = PhoneNumber('+' + phoneNum).getRegionCode() // -> 'SE'
    // todo 为什么要多走这一步呢？手机号格式有问题吗？
    let pn = new PhoneNumber(phoneNum.toString(), c)
    let region = pn.getRegionCode() ? pn.getRegionCode() : ''
    let countryCode = pn.getCountryCode() ? pn.getCountryCode().toString() : ''
    let countryName = getCountryNameByDomain(region) ? getCountryNameByDomain(region) : ''
    participantsInfoList[i]['countryCode'] = countryCode
    participantsInfoList[i]['country'] = countryName
    participantsInfoList[i]['phoneNum'] = '+' + phoneNum
  }
...
function getAllPhoneNumberList(groupInfos) {
  // 获取当前的联系人电话
...
export { getCountryByPhoneNum, getAllPhoneNumberList, getCountryNameByDomain }
